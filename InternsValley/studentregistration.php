...
<meta charset = "UTF-8" />
<meta name = "description" content = "Student Registration Page for InternsValley" />
<meta name = "keywords" content = "Student Registration Page, Interns Valley" />
...
<!--
<script>
//Next leads to next page i.e studentregistrationform.php
function nextpage() {
	document.window.location.href = "studentregistrationform.php";
}
...
if($_SERVER["REQUEST_METHOD"]=="POST") {
	...
	if($emailerr=="" && $cpwderr=="") {
		...
		try {
			...
			$stmt=$conn->prepare("SELECT email, username FROM studentlogin");
			$stmt->execute();
			...
			if( $emailerr=="" && $unameerr=="" ) {
				$sql = "INSERT INTO studentlogin (email, username, password ) VALUES (\"$email\", \"$uname\", \"$pwd\")";
				$conn->exec($sql);
				$status = true;
			 }
...
<?php
	if( $status === true ) {
		...
		//echo "<script> window.location.href = \"studentregistrationform.php\" </script><br />";
